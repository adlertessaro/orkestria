...
import { Eye, Search, Loader2, Clock, User, Building2, Package, MessageSquare, Users, FileText, Filter } from "lucide-react"
...
export default function ExibirPage() {
  ...
  return {
    ...
    descricao: extractFull("**Descrição**") // Aqui usamos a função que não corta linhas
  }
}
  ...
  return (
    <div className="p-6 space-y-6 bg-background min-h-screen">
      <div className="flex flex-col gap-1">
        <h1 className="text-3xl font-bold text-foreground">HOS Helper</h1>
        <h3 className="text-sm text-muted-foreground">Auxílios para suporte e implantação | Comunicação Interna</h3>
            ...
            {cards.map(card => {
              ...
              return (
                <TableRow key={card.id_trello} className="hover:bg-zinc-50/50">
                  <TableCell>
                    <Sheet onOpenChange={(open) => open && handleOpenSheet(card.id_trello)}>
                      ...
                      <SheetContent className="sm:max-w-xl overflow-y-auto">
                        ...
                        {/* GRID DE TOPO: Responsável e Solicitante */}
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="space-y-4">
                            ...
                            {loadingDetails ? (
                              <div className="flex justify-center py-10"><Loader2 className="animate-spin h-6 w-6 text-zinc-200" /></div>
                            ) : (
                              <div className="space-y-3">
                                ...
                                {details.comments.length === 0 && <p className="text-xs text-zinc-400 italic text-center py-4">Sem comentários.</p>}
                              </div>
                            )}
                          </div>
                        </div>
                  ...
                  <TableCell><Badge variant="outline">{card.status}</Badge></TableCell>
                </TableRow>
              )
            })}
          </TableBody>
